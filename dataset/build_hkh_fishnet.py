...
MANIFEST_JSON = DATASET_DIR / "hkh_rebuild_manifest.json"
FISHNET_GEOJSON = DATASET_DIR / "hkh_fishnet.geojson"
DEFAULT_OUTPUT_ROOT = Path(os.environ.get("HKH_RAW_ROOT", "data/raw"))
DEFAULT_TEMPLATE_DIR = DEFAULT_OUTPUT_ROOT / "Landsat7_2005"
TARGET_DIR_FULL8 = DEFAULT_OUTPUT_ROOT / "HKH_c02_current_raw_scenes/targets"
DONOR_DIR_FULL8 = DEFAULT_OUTPUT_ROOT / "HKH_c02_current_raw_scenes/donors"
...
def load_targets_meta(variant_name: str) -> dict[int, dict[str, Any]]:
    targets, _donors = resolve_manifest_variant(variant_name)
    return targets
...
def load_slate(variant_name: str) -> dict[int, list[DonorInfo]]:
    _targets, manifest_donors = resolve_manifest_variant(variant_name)
    out: dict[int, list[DonorInfo]] = defaultdict(list)
    for tid, rows in manifest_donors.items():
        for row in rows:
            path = DONOR_DIR_FULL8 / row["filename"]
            if not path.exists():
                continue
            out[tid].append(
                DonorInfo(
...
def target_path(target: dict[str, Any]) -> Path:
    return TARGET_DIR_FULL8 / target["filename"]
...
def write_variant_metadata(
    ...
    for variant in variants:
        ...
        policy = {
            ...
            "selection_manifest": str(MANIFEST_JSON),
            "provenance_enabled": not bool(getattr(args, "no_provenance", False)),
            "provenance_bands": PROVENANCE_BANDS,
            "provenance_semantics": {
                "target_id": "ID from dataset/hkh_rebuild_manifest.json for the target Landsat scene whose domain produced this pixel",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Canonical targets, donor selections, and dataset variants.",
    )
    parser.add_argument(
        "--dataset-variant",
        default="c02_current",
        help="Manifest dataset variant to resolve.",
    )
    parser.add_argument(
    ...
    targets = load_targets_meta(args.dataset_variant)
    slate = load_slate(args.dataset_variant)
