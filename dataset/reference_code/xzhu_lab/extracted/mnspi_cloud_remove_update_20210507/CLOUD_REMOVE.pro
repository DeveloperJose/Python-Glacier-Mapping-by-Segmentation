...
;         2017-11-21：correct a nonstop loop bug. It happens when processing large cloud patches and 
;                     no similar pixel selected for a cloudy pixel
;         2021-05-09：correct a bug of similar pixel searching for line-shape clouds  
;                          
...
;------------------------------------------------
 ...
 DN_min=0.0     ;set the range of DN
 DN_max=1.0
         ...
         ;neigborhood of cloud area: rectangle which can completely cover cloud
          ...
          sub_cloud=cloud[left_region:right_region,up_region:down_region] 
           ...
           for ic=0.0,num_cloud-1.0,1.0 do begin
             ...
             ri=float(fix(ind_cloud[ic]/(a_region)))    ; the location of target pixel
             ci=float(ind_cloud[ic]-(a_region)*ri)
             ...
             sub_on_common=fltarr(num_clear,nb)
             for iband=0,nb-1,1 do begin
               sub_on_common[*,iband]= fine2_clear[*,iband]-sub_fine2[ci,ri,iband]
             endfor
             rmsei=mean(sub_on_common^2,DIMENSION=2)^0.5+0.0001

             order_clear=sort(rmsei,/L64)  ;order all common pixels by rmse with the target pixel
             ...
             maxi_num_seach=min([num_clear,20.0*min_pixel])        ;limit the searching pixels to increase the efficiency
             while (isimilar le min_pixel-1 and iclear le maxi_num_seach-1.0 ) do begin
               indicate_similar=0
               for iband=0,nb-1,1 do begin
...
;   out_name=Dialog_PickFile(Title = 'Enter the filename of the restored image')
    out_name=FileName1+'cloud_remove'
