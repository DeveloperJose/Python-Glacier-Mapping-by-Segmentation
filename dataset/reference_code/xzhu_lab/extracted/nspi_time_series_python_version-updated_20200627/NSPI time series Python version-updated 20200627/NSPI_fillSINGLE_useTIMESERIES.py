...
# Please set the appropriate parameters for your own data in parameters_fillsingle.yaml before open this file!
...
Target = param['Target']   # set the target image in the time series to be reconstructed
...
for isub in range(0, n_nl * n_ns):
    ...
    fine0 = fine1_all[(Target-1)*num_band:Target*num_band, location[isub, 2]:location[isub, 3] + 1, location[isub, 0]:location[isub, 1] + 1]

    FileName = temp_file + '\\temp_mask' + str(isub + 1) + suffix
    _, _, mask = read_raster(FileName)

    # place the mark value
    mark = np.zeros((1, location[isub, 3] - location[isub, 2] + 1, location[isub, 1] - location[isub, 0] + 1)).astype(int)
    ...
    for i_img in range(Target-1, Target):
        ...
        while note_finish != 1 and i_input <= n_image - 1:
            ...
            for i in range(location[isub, 0], location[isub, 1] + 1):
                for j in range(location[isub, 2], location[isub, 3] + 1):
                    ...
                    if 0 < gap[j, i] < 10:  # only process contaminated pixels exclude background pixels
                        if mask2[j, i] == 0:  # if the corresponding pixel avalible,use this input image
                            ...
                            if num_outcloud > 0:
                                max_window_p = np.min(spatial_position[ind_outcloud])  # find the possible maximum half window
                            ...
                            if c_common >= 2.0 * min_similar:  # if same-class pixels more than required sample size
                                ...
                                for iband in range(0, num_band):
                                    ...
                                    if DN_min < predict_2 < DN_max:
                                        fine0[iband, j - location[isub, 2], i - location[isub, 0]] = W_T1 * predict_1 + W_T2 * predict_2
                                    else:
                                        fine0[iband, j - location[isub, 2], i - location[isub, 0]] = predict_1
                                note = 1
                                mark[0, j - location[isub, 2], i - location[isub, 0]] = 1 + 10 * i_input
                                gap[j, i] = -2
                                ...
                                if c_common >= 3 and extent >= max_window:
                                    ...
                                    for iband in range(0, num_band):
                                        ...
                                        if DN_min < predict_2 < DN_max:
                                            ...
                                            fine0[iband, j - location[isub, 2], i - location[isub, 0]] = predict_1
                                    mark[0, j - location[isub, 2], i - location[isub, 0]] = 2 + 10 * i_input
                                    note = 1
                                    gap[j, i] = -2
                                else:
                                    ...
                                    if num_good_before > 0 and num_good_after > 0:
                                        ...
                                        for iband in range(0, num_band):
                                            ...
                                            fine0[iband, j - location[isub, 2], i - location[isub, 0]] = (T2 * before_band + T1 * after_band) / (T2 + T1)
                                    else:
                                        fine0[:, j - location[isub, 2], i - location[isub, 0]] = fine2[:, j, i]

                                    mark[0, j - location[isub, 2], i - location[isub, 0]] = 3 + 10 * i_input
                                    note = 1
                                    gap[j, i] = -2
...
path = os.path.splitext(path1)[0] + "_fill_NSPI2" + suffix
if suffix == '.tif':
    driver = gdal.GetDriverByName("GTiff")
elif suffix == "":
    driver = gdal.GetDriverByName("ENVI")
dataset = driver.Create(path, orig_ns, orig_nl, num_band, gdal.GDT_Float32)
i = 0
for data in datalist:
...
n_mask = 1
in_ds = gdal.Open(path1)
path = os.path.splitext(path1)[0] + "_fill_quality_flag2" + suffix
if suffix == '.tif':
    driver = gdal.GetDriverByName("GTiff")
elif suffix == "":
    driver = gdal.GetDriverByName("ENVI")
