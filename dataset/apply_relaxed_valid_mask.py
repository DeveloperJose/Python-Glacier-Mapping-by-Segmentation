#!/usr/bin/env python3
"""Create the documented full8 fishnet datasets with relaxed valid masks.
...
def load_fishnet6():
    path = REPO_ROOT / "dataset/build_hkh_fishnet.py"
...
def write_target_data_present_to_tile(
    fishnet6, tile, target_meta, target_path: Path
) -> int:
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=REPO_ROOT / "dataset/hkh_rebuild_manifest.json",
    )
    parser.add_argument("--dataset-variant", default="c02_current")
    ...
    TEMPLATE_DIR = args.template_dir or RAW_ROOT / "Landsat7_2005"
    ...
    if unknown:
        raise ValueError(
            f"Unknown variants: {unknown}. Choices: {sorted(VARIANT_SUFFIXES)}"
        )

    fishnet6 = load_fishnet6()
    fishnet6.MANIFEST_JSON = args.manifest
    ...
    targets = fishnet6.load_targets_meta(args.dataset_variant)
    ids = sorted(targets)
    tile_indices = None
    ...
    for tid in ids:
        ...
        with rasterio.open(target_path) as src:
            scene_profile = src.profile.copy()
        scene_tiles = [
            t for t in templates if fishnet6.scene_intersects_tile(scene_profile, t)
        ]
        print(f"ID {tid:02d} {target_path.name}: tiles={len(scene_tiles)}", flush=True)
        for tile in scene_tiles:
            written = write_target_data_present_to_tile(
                fishnet6, tile, target_meta, target_path
            )
            rows.append(
                {
                    "target_id": tid,
    ...
    for tile in templates:
        ...
        row = {"tile": tile.index}
        row.update(
            {f"{k}_existing_fill_pixels_preserved": v for k, v in counts.items()}
        )
    ...
    for key in CURRENT_VARIANT_KEYS:
        ...
        manifest_rows = [
            {
                "image": f"image{tile.index}.tif",
                "tile_index": tile.index,
                "template": str(tile.path),
            }
            for tile in templates
        ]
