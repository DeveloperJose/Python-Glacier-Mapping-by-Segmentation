...
# Threshold for considering a file "clean" - values above this trigger regeneration
DEFAULT_SKIP_THRESHOLD = 25000.0  # m/yr
...
def is_valid_velocity_file(
    ...
    """
    Check if velocity file exists and has reasonable values.

    Returns False if:
    - File doesn't exist
    - Any velocity component has max >= threshold (likely corrupted/clipped)
    """
    if not path.exists():
        return False
    try:
        with rasterio.open(path) as src:
            data = src.read()
            # Check v, vx, vy bands (indices 0, 1, 2)
            for i in range(3):
                if np.nanmax(np.abs(data[i])) >= threshold:
                    return False
        return True
    except Exception:
        return False
...
def find_overlapping_datacubes(
    image_bbox_latlon: Tuple[float, float, float, float], catalog: List[Dict]
) -> List[Dict]:
    """
...
def get_image_bbox_latlon(
    bounds: Tuple[float, float, float, float], epsg_code: int
...
def process_single_image(args: Tuple[int, Path, Path, List[Dict], List[int]]) -> Dict:
    """
    ...
    try:
        ...
        if not overlapping:
            raise ValueError(
                f"No overlapping datacubes found for image at {image_bbox_latlon}"
            )
        ...
        # Filter extreme outliers before filling NaN
        # Physically reasonable glacier velocities: -50,000 to +50,000 m/year
        # (allows for extreme surging glaciers, but filters corrupted ITS_LIVE data)
        MAX_VELOCITY = 50000.0  # m/year

        v_clipped = np.clip(v_resampled, 0, MAX_VELOCITY)
        vx_clipped = np.clip(vx_resampled, -MAX_VELOCITY, MAX_VELOCITY)
        vy_clipped = np.clip(vy_resampled, -MAX_VELOCITY, MAX_VELOCITY)

        # Fill NaN→0 (consistent with slice.py pattern)
        v_filled = np.nan_to_num(v_clipped, nan=0.0)
        vx_filled = np.nan_to_num(vx_clipped, nan=0.0)
        vy_filled = np.nan_to_num(vy_clipped, nan=0.0)
        ...
        with rasterio.open(output_path, "w", **output_meta) as dst:
            ...
            dst.set_band_description(
                1, f"velocity_magnitude_median_{years[0]}_{years[-1]}"
            )
...
def main():
    ...
    parser.add_argument(
        "--start-year", type=int, default=2002, help="Start year for velocity median"
    )
    parser.add_argument(
        "--end-year",
        type=int,
    ...
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Skip files that already exist and have max velocity < skip-threshold",
    )
    parser.add_argument(
        "--skip-threshold",
        type=float,
        default=DEFAULT_SKIP_THRESHOLD,
        help=f"Threshold for --skip-existing: regenerate files with max >= this value (default: {DEFAULT_SKIP_THRESHOLD} m/yr)",
    )
    ...
    catalog_path = (
        args.catalog if hasattr(args, "catalog") and args.catalog else CATALOG_PATH
    )
    ...
    if args.skip_existing:
        ...
        for img_path in images:
            output_path = args.output_dir / img_path.name
            if is_valid_velocity_file(output_path, args.skip_threshold):
                skipped += 1
            else:
                images_to_process.append(img_path)
        images = images_to_process
        logger.info(
            f"--skip-existing (threshold={args.skip_threshold}): "
            f"{skipped} valid files skipped, {len(images)} to process"
        )
