...
if __name__ == "__main__":
    ...
    if conf.training_opts.fine_tune:
        ...
        frame = Framework.from_checkpoint(final_model_path, device=int(conf.training_opts.gpu_rank))
        frame.freeze_layers()
    ...
    fn.print_conf(conf)
    fn.log(logging.INFO, f"# Training Instances = {len(train_loader)}, # Validation Instances = {len(val_loader)}")
    ...
    _normalize = np.load(pathlib.Path(conf.loader_opts.processed_dir) / "normalize_train.npy")
    if conf.loader_opts.normalize == "min-max":
        _normalize = (_normalize[2][conf.loader_opts.use_channels],
                      _normalize[3][conf.loader_opts.use_channels])
    elif conf.loader_opts.normalize == "mean-std":
        _normalize = (np.append(_normalize[0], 0)[conf.loader_opts.use_channels],
