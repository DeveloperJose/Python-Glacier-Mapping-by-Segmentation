...
def upload_single_run(run_dir: Path, tracking_uri: str, force: bool = False) -> bool:
    ...
    try:
        ...
        if existing_runs and not force:
            print(f"Skipping {run_dir.name}: Run already exists in MLflow (use --force to override)")
            return False
        
        # Start run
        with mlflow.start_run(run_name=run_name) as run:
            # Log parameters
            params = MLflowManager.extract_mlflow_params(config, {}) if MLFLOW_AVAILABLE else {}
...
def upload_batch_runs(output_dir: Path, tracking_uri: str, experiment_type: Optional[str] = None, force: bool = False) -> int:
    """Upload multiple runs to MLflow."""
    print(f"Scanning for runs in: {output_dir}")
    
    # Find all run directories
    run_dirs = [d for d in output_dir.iterdir() if d.is_dir() and (d / "conf.json").exists()]
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="output", help="Output directory containing runs")
    parser.add_argument("--tracking-uri", type=str, default="https://mlflow.developerjose.duckdns.org/", help="MLflow tracking URI")
    parser.add_argument("--experiment-type", type=str, help="Filter by experiment type (e.g., baseline_ci, baseline_debris)")
    parser.add_argument("--force", action="store_true", help="Override existing runs")
    parser.add_argument("--batch", action="store_true", help="Upload all runs from output directory")
    ...
    if args.batch:
        ...
        successful_uploads = upload_batch_runs(output_dir, args.tracking_uri, args.experiment_type, args.force)
