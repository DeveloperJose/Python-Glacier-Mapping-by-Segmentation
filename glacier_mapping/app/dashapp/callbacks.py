...
def register_callbacks(app, obj, layout):
    @app.callback(Output("x-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        ...
        fig = (fig - np.min(fig, axis=(0, 1))) / (
            np.max(fig, axis=(0, 1)) - np.min(fig, axis=(0, 1))
        )
        fig = px.imshow(fig)
    ...
    @app.callback(Output("y-true-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        if active_cell is None:
            row_id = 1
        else:
        ...
        fig.update_yaxes(showticklabels=False)
        fig.update_traces(
            dict(showscale=False, coloraxis=None), selector={"type": "heatmap"}
        )
        return dcc.Graph(figure=fig, id="y-true")

    @app.callback(Output("y-pred-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        if active_cell is None:
            row_id = 1
        else:
    ...
    @app.callback(
        Output("precision-div", "children"), Input("main-table", "active_cell")
    )
    def display_click_data(active_cell):
        if active_cell is None:
            row_id = 1
        ...
        debris_precision = str(
            np.round(obj.get_df().iloc[row_id]["debris_precision"] * 100, 2)
        )
        return f"Clean Ice: {ci_precision}%, Debris: {debris_precision}%"

    @app.callback(Output("recall-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        if active_cell is None:
            row_id = 1
        else:
        ...
        debris_recall = str(
            np.round(obj.get_df().iloc[row_id]["debris_recall"] * 100, 2)
        )
        return f"Clean Ice: {ci_recall}%, Debris: {debris_recall}%"

    @app.callback(Output("iou-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        if active_cell is None:
            row_id = 1
        else:
    ...
    @app.callback(
        Output("runs-dropdown", "options"), Input("processed-dirs-dropdown", "value")
    )
    def on_processed_dropdown_click(value):
        obj.set_processed_folder(value)
        return obj.get_all_preds_folders()

    @app.callback(Output("main-table", "data"), Input("runs-dropdown", "value"))
    ...
    @app.callback(
        Output("scatter-plot-div", "children"), Input("runs-dropdown", "value")
    )
    ...
    @app.callback(Output("roc-div", "children"), Input("main-table", "active_cell"))
    def display_click_data(active_cell):
        classes = {0: "Background", 1: "Clean Ice", 2: "Debris"}
        if active_cell is None:
            row_id = 1
        else:
            row_id = active_cell["row_id"]
