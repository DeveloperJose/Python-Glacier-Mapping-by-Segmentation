...
def find_lr(module: pl.LightningModule, train_loader, init_value: float, final_value: float) -> Tuple[List[float], List[float]]:
    ...
    for i, data in enumerate(tqdm(train_loader, desc="LR Finder")):
        ...
        loss = module.compute_loss(outputs, labels.permute(0, 3, 1, 2), labels.squeeze(-1))
...
def lr_finder(module: pl.LightningModule, train_loader, init_value: float = 1e-9, final_value: float = 1.0) -> Tuple[List[float], List[float]]:
