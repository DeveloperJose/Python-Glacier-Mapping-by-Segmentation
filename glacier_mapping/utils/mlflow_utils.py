...
class MLflowManager:
    """Main MLflow management class with static methods."""
    ...
    def load_server_config(servers_yaml_path: str, server_name: str) -> dict:
        ...
        if not Path(servers_yaml_path).exists():
            raise FileNotFoundError(f"Server configuration file not found: {servers_yaml_path}")
    ...
    def categorize_experiment(config: dict) -> str:
        ...
        if any(pattern in dataset_name for pattern in ['w256', 'w1024', 'o32', 'o128', 'f20', 'f15']):
    ...
    def generate_run_tags(config: dict, server_config: dict, config_file: str) -> dict:
        ...
        tags = {
            'experiment_type': 'physics' if any(ch >= 16 for ch in loader_opts.get('use_channels', [])) else 'baseline',
            'target_class': MLflowManager._get_target_class_name(output_classes),
    ...
    @staticmethod
    def setup_mlflow_logger(tracking_uri: str, experiment_name: str, run_name: str, tags: dict):
    ...
    def attempt_connection_with_retry(
        ...
        try:
            ...
            warnings.warn(f"MLflow connection failed (attempt {failed_attempts + 1}/{max_retries}): {e}")
    ...
    @staticmethod
    def log_metrics_safe(metrics: dict, step: Optional[int] = None, logger_name: str = "MLflow"):
    ...
    @staticmethod
    def log_artifact_safe(local_path: str, artifact_path: Optional[str] = None, logger_name: str = "MLflow"):
