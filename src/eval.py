...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(
        columns=["Filename", "IoU", "Precision", "Recall", "Tp", "Fp", "Fn"]
    )
    if conf.label == "water":
        invert = False
    ...
    new_row = {
        "Filename": "Mean",
        "IoU": np.round(
            np.sum(np.asarray(df.Tp))
            / (
                np.sum(np.asarray(df.Tp))
                + np.sum(np.asarray(df.Fp))
                + np.sum(np.asarray(df.Fn))
            ),
            4,
        ),
        "Precision": np.round(
            np.sum(np.asarray(df.Tp))
            / (np.sum(np.asarray(df.Tp)) + np.sum(np.asarray(df.Fp))),
            4,
        ),
        "Recall": np.round(
            np.sum(np.asarray(df.Tp))
            / (np.sum(np.asarray(df.Tp)) + np.sum(np.asarray(df.Fn))),
            4,
        ),
