...
if __name__ == "__main__":
    ...
    frame: Framework = Framework.from_checkpoint(
        checkpoint_path, device="cpu", testing=True
    )

    # % Load data stuff
    tiff_path = pathlib.Path("/home/jperez/data/HKH_raw/Landsat7_2005/")
    dem_path = pathlib.Path("/home/jperez/data/HKH_raw/DEM/")
    labels_fname = pathlib.Path(
        "/home/jperez/data/HKH_raw/labels/HKH_CIDC_5basins_all.shp"
    )
    ...
    def process(idx, fname):
        ...
        pred_tiff = np.transpose(read_tiff(pred_fname).read(), (1, 2, 0)).astype(
            np.uint8
        )
