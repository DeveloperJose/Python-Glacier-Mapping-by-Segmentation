...
class Base:
    def __init__(self):
        ...
        self.preds_folder = "combined_0"
        self.preds_dir = (
            self.data_dir / self.processed_folder / "preds" / self.preds_folder
        )
    ...
    def get_all_processed_folders(self):
        self.all_processed_folders = sorted(
            [x for x in os.listdir(self.data_dir) if "processed" in x]
        )
    ...
    def get_all_preds_folders(self):
        self.all_preds_folders = sorted(
            [x for x in os.listdir(self.processed_dir / "preds")]
        )
